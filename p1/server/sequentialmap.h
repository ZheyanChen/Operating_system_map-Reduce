...
public:
  ...
  virtual bool insert(K key, V val, std::function<void()> on_success) {
    pair<K,V> con(key,val);
    bool check = true;
    //list<pair<K, V>>::iterator it;
    
    for (pair<K,V> &c : entries /*auto it = entries.begin(); it != entries.end(); it++*/)
    {
      //pair<K,V> c = *it;
      if( c.first==key)
      {
  ...
  virtual bool upsert(K key, V val, std::function<void()> on_ins,
    ...
    pair<K,V> tbrm;
    bool check = true;
    for ( /*auto it = entries.begin(); it != entries.end(); it++*/ pair<K,V> &c : entries)
    {
      //pair<K,V> c = *it;
      if(c.first==key)
      {
        //c=make_pair(c.first,val);
        c.second=val;
        tbrm = make_pair(c.first,c.second);
        check=false;
      }
    }
    //
