...
int create_server_socket(std::size_t port) {
  ...
  addr.sin_family = AF_INET;   // give an option of server sockets
  addr.sin_addr.s_addr = htonl(INADDR_ANY);  // any address in virtual memory
  addr.sin_port = htons(port);
  if (bind(sd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
    close(sd);
    error_message_and_exit(0, errno, "Error binding socket to local address: ");
  }
  if (listen(sd, 0) < 0) {  // throw listening err
    close(sd);
 ...
 */
void echo_server(int sd, bool verbose) {    // 1st arg ==> sd generated after successful connection
  ...
  while (true) {
    ...
    ssize_t rcd = recv(sd, buf, sizeof(buf), 0);      //recv function receives a message from a connected socket to buf
    if (rcd <= 0) {
      if (errno != EINTR) {  //check whether recv function was interrupted
        if (rcd == 0) {
          break;   // no msg available or peer has performed a shutdown
        } else {
      ...
      xmitBytes += rcd;  
      char *next_byte = buf;   
      std::size_t remain = rcd;  // remain == actual length of the msg
      while (remain) {
        std::size_t sent = send(sd, next_byte, remain, 0);    // send func transmit from 2nd arg to 1st arg
        if (sent <= 0) {
          if (errno != EINTR) {
          ...
          next_byte += sent;
          remain -= sent;       // decrement bytes in case of "shortcount"
        }
      }
